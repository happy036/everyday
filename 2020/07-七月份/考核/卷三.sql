...
ALTER TABLE customer MODIFY COLUMN email VARCHAR(20) NOT NULL;
...
SELECT pname,pid FROM product WHERE pid IN (SELECT pid FROM orderitem WHERE cid=(SELECT cid FROM customer WHERE cname='张三'));
...
SELECT c.cid,cname,pid FROM customer AS c INNER JOIN orderitem AS o ON c.cid=o.cid ;
...
SELECT * FROM product WHERE pid IN(SELECT pid FROM orderitem WHERE cid IN (SELECT cid FROM customer WHERE age BETWEEN 30 AND 50));
# 50.查询每个商品的平均订购的数量
