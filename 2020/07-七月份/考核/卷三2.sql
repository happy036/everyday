...
INSERT INTO customer VALUES("c0007",'拉拉','女',18);
...
SELECT (`count`*price)AS 总计,COUNT FROM customer AS c JOIN orderitem AS o JOIN product AS p ON c.cid=o.cid AND o.pid=p.pid WHERE cname='刘二' AND pname='沙发';
...
SELECT pid,pname FROM product WHERE pid IN (SELECT pid FROM orderitem WHERE cid=(SELECT cid FROM customer WHERE cname='张三'));
...
SELECT price FROM product WHERE pid IN('p001','p002');
...
SELECT*FROM orderitem WHERE MONTH(ordertime)=4 AND `count`>2;
# 35.查询性别是男并且年龄大于30的顾客的信息
SELECT*FROM customer WHERE sex='男'AND age>30;
# 36.查询年龄相同的顾客的信息
SELECT*FROM customer AS c ,customer AS d WHERE c.age=d.age AND c.cid<>d.cid;
...
SELECT c.cid,cname,pid FROM customer AS c JOIN orderitem AS o ON c.cid=o.cid;
...
SELECT pname,price FROM product WHERE pid IN(SELECT pid FROM orderitem WHERE cid IN(SELECT cid FROM customer WHERE age BETWEEN 30 AND 40));
# 47.查找女顾客购买的商品号，商品名和价格


SELECT pid,pname,price FROM product WHERE pid IN(SELECT pid FROM orderitem WHERE cid IN(SELECT cid FROM customer WHERE sex='女'));
# 48.查询单价大于2000或者是库存大于20的商品的信息
SELECT*FROM product WHERE price>2000 OR stock>20;
# 49.查询是女顾客并且年龄段在30到50之间购买的商品信息


SELECT*FROM orderitem WHERE pid IN(SELECT pid FROM orderitem WHERE cid IN(SELECT cid FROM customer WHERE sex='女'AND age BETWEEN 30 AND 50));
# 50.查询每个商品的平均订购的数量
