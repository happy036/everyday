...
sname VARCHAR(20) PRIMARY KEY NOT NULL DEFAULT ''  COMMENT'姓名',
...
SELECT cno FROM score WHERE sno=103 ORDER BY cno DESC;     
...
SELECT SUM(degree),AVG(degree) FROM score GROUP BY cno,sno;
# 38.查询Score表中每门课程的平均分数（要求查询的课程至少要有3个成绩）。
SELECT cno,AVG(degree) FROM score GROUP BY cno HAVING COUNT(degree)>=3;
...
SELECT tname,depart FROM teacher WHERE tsex='男'; 
# 45.查询学生”陆君”所选每门课的平均分
SELECT AVG(degree) FROM score WHERE sno=(SELECT sno FROM student WHERE sname='陆君') GROUP BY cno; 
# 46.查询选了课程编号为3-245且课程成绩在70分以上的学生的学号和姓名
SELECT st.sno,sname FROM score AS s JOIN student AS st ON s.sno=st.sno WHERE cno='3-245'AND degree>70;
# 47.查询平均成绩大于70 的所有学生的学号、姓名和平均成绩
SELECT st.sno,sname,AVG(degree) FROM student AS st JOIN score AS s ON st.sno=s.sno GROUP BY sno HAVING AVG(degree)>70;
# 48.查询所有学生的选课情况(学生姓名和课程名和老师名)
SELECT sname,depart,tname FROM student AS st JOIN teacher AS t JOIN score AS sc JOIN course AS c ON st.sno=sc.sno AND sc.cno=c.cno AND c.tno=t.tno;
# 49.查询选修"计算机导论"课程的学生的姓名和学号以及成绩
SELECT sname,st.sno,degree FROM student AS st JOIN score AS sc ON st.sno=sc.sno WHERE cno=(SELECT cno FROM course WHERE cname='计算机导论');
# 50.查询没有的学生的老师信息（使用外连接，子查询都可以）
SELECT*FROM teacher WHERE tno=(SELECT tno FROM course WHERE cno NOT IN(SELECT DISTINCT cno FROM score));