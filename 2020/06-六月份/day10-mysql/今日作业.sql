...
SELECT cno,degree FROM score GROUP BY cno HAVING degree>85;
...
SELECT sno,cno,degree FROM score ORDER BY cno ,degree DESC;
#11、 查询“95031”班的学生人数。
...
SELECT sname,cname,degree FROM course,score,student WHERE score.cno=course.cno AND student.sno=score.sno;
#24、查询“95031”班学生的平均分。

SELECT AVG(degree) FROM score,student WHERE student.sno=score.sno AND class=95031;
...
SELECT sno,sname,sbirthday FROM student WHERE YEAR(sbirthday)=(SELECT YEAR(sbirthday)FROM student WHERE sno=108) OR YEAR(sbirthday)=(SELECT YEAR(sbirthday)FROM student WHERE sno=101);
33、查询“张旭“教师任课的学生成绩。
...
SELECT tanme FROM teacher WHERE
