...
SELECT empno,AVG(sal)FROM emp WHERE sal>1200 GROUP BY deptno HAVING AVG(sal)>1500 ORDER BY AVG(sal) DESC ;
...
SELECT ename,hiredate FROM emp WHERE deptno=(SELECT deptno FROM emp WHERE ename='Scott') AND NOT(ename='Scott');
...
SELECT empno,ename,sal FROM emp AS e,(SELECT  deptno,AVG(sal) AS asal FROM emp GROUP BY deptno)AS f WHERE e.deptno=f.deptno AND e.sal>f.asal;

#14.	查询和姓名中包含字母u的员工在相同部门的员工的员工号和姓名
SELECT*FROM emp WHERE deptno=(SELECT deptno FROM emp WHERE ename LIKE '%u%');
...
SELECT dname,loc,COUNT(*),AVG(sal) FROM emp,dept WHERE emp.deptno=dept.deptno GROUP BY dname;