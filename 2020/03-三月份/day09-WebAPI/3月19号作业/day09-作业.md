...
```js
DOMContentLoaded事件:等网页中的DOM结构加载完就可以执行
onload事件：等网页中的所有东西(结构 样式 js代码 img 文字 各种文件)加载完毕 才能执行js代码
```
第3题. 定时器分为几种，有什么区别，怎么清除定时器

```js
延时定时器：等待一段时间 来执行某个动作  clearTimeout(定时器的名字)
周期定时器：隔一段时间 执行一个动作  clearInterval(定时器的名字)
```
