      ...
      <div class="container">
              ...
              <tr v-else v-for="goods in effectiveGoodsList" :key="goods.id">
                ...
                <td class="tc">
                  <XtxNumberBox
                    ...
                    @update:modelValue="changeGoodsCount(goods.skuId, $event)"
                  ></XtxNumberBox>
                </td>
                <td class="tc">
                  <p class="f16 red">
                ...
                <td class="tc">
                  ...
                  <p>
                    <a
                      class="green"
                      href="javascript:"
                      @click="deleteGoodsOfCart(goods.skuId)"
                      >删除</a
                    >
                  </p>
        ...
        <div class="action">
          <div class="batch">
            ...
            <a
              href="javascript:"
              @click="deleteGoodsEmptyInvalidGoods('userSelectedGoodsList')"
              >删除商品</a
            >
            <a href="javascript:">移入收藏夹</a>
            <a
              href="javascript:"
              @click="deleteGoodsEmptyInvalidGoods('invalidGoodsList')"
              >清空失效商品</a
            >
          </div>
          <div class="total">
  ...
  setup() {
    ...
    const deleteGoodsOfCart = (skuId) => {
      Confirm({
        ...
        .catch(() => {
          Message({ type: "success", text: "取消删除商品成功" });
        });
    };
    // 批量删除商品和清空失效商品
    const deleteGoodsEmptyInvalidGoods = (flag) => {
      // 弹框提示文字
      let content = "";
      ...
      Confirm({ content })
        .then(() => {
          // 执行操作
          store.dispatch("cart/deleteGoodsEmptyInvalidGoods", flag);
        })
        .catch(() => {});
    };
    // 更改购物车中的商品数量
    const changeGoodsCount = (skuId, count) => {
