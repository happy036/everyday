...
function setDefaultSelected(skuId, skus, specs) {
  if (!skuId) return;
  // skuId 规格ID,skus 当前商品所有可组合的规格集合,specs 即将被设置状态的数据
...
function sendDataToParent(specs, pathMap, skus, emit) {
  ...
  // 判断用户选择的规格是否是完整的 如果是完整的才需要将数据传递到父组件
  if (selected.length === specs.length) {
    ...
    emit("onSpecChanged", {
      ...
      // 用户选择的规格名称
      attrsText: target.specs.map((spec) => `${spec.name}:${spec.valueName}`),
