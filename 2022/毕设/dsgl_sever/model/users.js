...
module.exports.register = async (username, password, mobile, email) => {
    return await query(`insert into user (username, password, mobile,email,role) values( "${username}", "${password}", "${mobile}", "${email}",'普通用户')`)
}
// 根据用户名查询用户
...
module.exports.allUsersLimit = async ({
    ...
    return await query(`select id,username,mobile,role,email from user order by id limit ${(pagenum - 1) * pagesize},${pagesize}`)
}
// 根据id查询用户
...
// 更新用户信息
module.exports.updateUser = async ({ id, username, mobile, email, role }) => {
    return await query(`UPDATE user
SET username=?, mobile=?, email=?, role=? WHERE id=${id}`, [username, mobile, email, role])
}
// 删除用户
